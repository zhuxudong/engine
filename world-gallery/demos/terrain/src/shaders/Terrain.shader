Shader "Terrain" {
  SubShader "Default" {
    Pass "Forward" {
      ...
      #include "Terrain/TerrainGeometryPass.glsl"
      ...
      vec4 shadeTerrain(Varyings varyings) {
        ...
        if (material_DebugView == 32) {
          float sourceHeight = smoothstep(-0.1, 2.0, 0.5 + h3 / 300.0);
          return vec4(vec3(sourceHeight), 1.0);
        }
        ...
        if (material_DebugView == 31) {
          return vec4(vec3(float(decodeNavigation(raw3))), 1.0);
        }
        #endif
    ...
    Pass "DepthPrimingOnly" {
      ...
      void depthFrag(Varyings varyings) {
        if (varyings.renderable < 1.0) discard;
